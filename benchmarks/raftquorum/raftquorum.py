...
# Suite ########################################################################
class RaftQuorumSuite(benchmark.Suite[Input, Output]):
    ...
    def run_benchmark(self,
        ...
        # Write config file.
        net = RaftQuorumNet(self._cluster, input)
        config = net.config()
        ...
        for (i, participant) in enumerate(net.placement().participants):
            p = bench.popen(
                ...
                cmd=java("100m") + [
                    ...
                    'frankenpaxos.raftquorum.ParticipantMain',
                    '--index',
                    str(i),
                    '--config',
                    config_filename,
        ...
        for (i, client) in enumerate(net.placement().clients):
            p = bench.popen(
                ...
                cmd=java("100m") + [
                    '-cp',
                    os.path.abspath(args['jar']),
                    'frankenpaxos.raftquorum.ClientMain',
                    '--host',
                    client.host.ip(),
                    '--port',
                    str(client.port),
        ...
        return RaftQuorumOutput(read_output = read_output,
